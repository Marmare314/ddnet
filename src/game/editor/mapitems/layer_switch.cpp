...
bool CLayerSwitch::IsEmpty(const std::shared_ptr<CLayerTiles> &pLayer)
{
	for(int y = 0; y < pLayer->m_Height; y++)
		for(int x = 0; x < pLayer->m_Width; x++)
			if(Editor()->m_AllowPlaceUnusedTiles || IsValidSwitchTile(pLayer->GetTile(x, y).m_Index))
				return false;
...
void CLayerSwitch::BrushDraw(const std::shared_ptr<CLayer> &pBrush, float wx, float wy)
{
	...
	{
		Editor()->m_SwitchNum = pSwitchLayer->m_SwitchNumber;
		Editor()->m_SwitchDelay = pSwitchLayer->m_SwitchDelay;
	}

	bool Destructive = Editor()->m_BrushDrawDestructive || IsEmpty(pSwitchLayer);
	...
	for(int y = 0; y < pSwitchLayer->m_Height; y++)
		for(int x = 0; x < pSwitchLayer->m_Width; x++)
		{
			...
			if((Editor()->m_AllowPlaceUnusedTiles || IsValidSwitchTile(pSwitchLayer->m_pTiles[y * pSwitchLayer->m_Width + x].m_Index)) && pSwitchLayer->m_pTiles[y * pSwitchLayer->m_Width + x].m_Index != TILE_AIR)
			{
				if(Editor()->m_SwitchNum != pSwitchLayer->m_SwitchNumber || Editor()->m_SwitchDelay != pSwitchLayer->m_SwitchDelay)
				{
					m_pSwitchTile[fy * m_Width + fx].m_Number = Editor()->m_SwitchNum;
					m_pSwitchTile[fy * m_Width + fx].m_Delay = Editor()->m_SwitchDelay;
				}
...
void CLayerSwitch::FillSelection(bool Empty, const std::shared_ptr<CLayer> &pBrush, CUIRect Rect)
{
	...
	for(int y = 0; y < h; y++)
	{
		for(int x = 0; x < w; x++)
		{
			...
			if(Empty || (!Editor()->m_AllowPlaceUnusedTiles && !IsValidSwitchTile((pLt->m_pTiles[SrcIndex]).m_Index)))
			{
			...
			else
			{
				...
				if(pLt->m_Switch && m_pTiles[TgtIndex].m_Index > 0)
				{
					if(!IsSwitchTileNumberUsed(m_pSwitchTile[TgtIndex].m_Type))
						m_pSwitchTile[TgtIndex].m_Number = 0;
					else if(pLt->m_pSwitchTile[SrcIndex].m_Number == 0 || Editor()->m_SwitchNum != pLt->m_SwitchNumber)
						m_pSwitchTile[TgtIndex].m_Number = Editor()->m_SwitchNum;
					else
